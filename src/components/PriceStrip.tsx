      ...
      {dates.map((d, i) => {
        ...
        return (
          <button
            ...
            onClick={() => onPick(i)}
            aria-pressed={isSel}
            className={`group relative flex min-w-[72px] flex-1 cursor-pointer flex-col items-center justify-center gap-1 py-2.5 transition-colors duration-300 sm:min-w-0 ${isSel ? (isLight ? 'bg-[#EFF6FF]' : 'bg-[#121E3C]') : (isLight ? 'bg-transparent hover:bg-[#F9FAFB]' : 'bg-transparent hover:bg-[#131c33]')}`}
          >
            <span className={`hidden max-w-full truncate text-[11px] leading-none transition-colors duration-300 sm:block ${isSel ? (isLight ? 'font-semibold text-[#111827]' : 'font-semibold text-[#7CC0FF]') : (isLight ? 'text-[#6B7280] group-hover:text-[#2563EB]' : 'text-[#9baec7] group-hover:text-[#e8c86a]')}`}>{d.label}</span>
            <span className={`block max-w-full truncate text-[11px] leading-none transition-colors duration-300 sm:hidden ${isSel ? (isLight ? 'font-semibold text-[#111827]' : 'font-semibold text-[#7CC0FF]') : (isLight ? 'text-[#6B7280] group-hover:text-[#2563EB]' : 'text-[#9baec7] group-hover:text-[#e8c86a]')}`}>{d.label.split(', ')[1]}</span>
            <span className={`max-w-full truncate px-0.5 text-[12px] font-bold leading-none transition-colors duration-300 sm:text-[13.5px] ${isSel ? (isLight ? 'text-[#2563EB]' : 'text-[#3B9CFF]') : (isLight ? 'text-[#111827] group-hover:text-[#2563EB]' : 'text-white group-hover:text-[#d4af37]')}`}>{stripInr(d.price)}</span>
