...
export const SidebarFilters = () => {
  ...
  return (
    <aside className={`group/sidebar ${filtersOpen ? 'flex' : 'hidden'} w-full shrink-0 flex-col border-r p-4 md:flex md:w-[300px] transition-colors duration-300 ${isLight ? 'bg-[#F7F9FC] border-r-[#E5E7EB]' : 'bg-[#0E1833] border-[rgba(212,175,55,0.25)]'}`}>
      <div className="flex items-center justify-between pb-2">
        <span className={`text-[11px] font-bold tracking-[0.14em] transition-colors duration-300 ${isLight ? 'text-[#111827]' : 'text-[#7CC0FF]'}`}>REFINE RESULTS</span>
        <button
          type="button"
          onClick={clearFilters}
          className={`cursor-pointer border-none bg-transparent text-[11.5px] font-medium transition-all duration-200 hover:text-[#d4af37] hover:drop-shadow-[0_0_6px_rgba(212,175,55,0.7)] ${isLight ? 'text-[#6B7280] hover:text-[#2563EB]' : 'text-[#9CC6FF]/85 hover:text-[#f5d67b]'}`}
        >
          clear all
        </button>
      </div>

      <div className={`pretty-scroll min-h-0 max-h-[50vh] flex-1 overflow-x-hidden overflow-y-scroll rounded-[16px] border px-4 py-1.5 md:max-h-none transition-colors duration-300 ${isLight ? 'bg-white border-[#E5E7EB] shadow-[0_2px_8px_rgba(0,0,0,0.06)]' : 'bg-[#0F1B3A] border-[rgba(124,192,255,0.35)] shadow-[inset_0_1px_0_rgba(255,255,255,0.02),0_0_18px_rgba(59,156,255,0.12)]'}`}>
