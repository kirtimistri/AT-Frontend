...
export const SkeletonCard = ({ index }: { index: number }) => {
  ...
  return (
    <div
      className={`card-shimmer relative flex min-h-[168px] animate-pulse flex-col overflow-hidden rounded-[12px] border p-4 pt-8 lg:flex-row transition-colors duration-300 ${isLight ? 'bg-white border-[#E5E7EB] shadow-[0_2px_8px_rgba(0,0,0,0.06)]' : 'bg-[#0f172a] border-[#214b7e]'}`}
    style={{ '--shimmer-delay': `${index * 180}ms`, animationDelay: `${index * 180}ms` } as CSSProperties}
  >
    {/* Badge placeholder */}
    <div className={`absolute left-[22px] top-[15px] h-[24px] w-[104px] rounded-full transition-colors duration-300 ${isLight ? 'bg-[#EFF6FF]' : 'bg-[#16304f]'}`} />
    {/* Left column */}
    <div className="min-w-0 flex-1 pr-0 lg:pr-6">
      {/* Airline row */}
      <div className="flex items-center gap-3.5">
        <div className="flex flex-col items-center">
          <div className={`h-[44px] w-[44px] rounded-full transition-colors duration-300 ${isLight ? 'bg-[#E5E7EB]' : 'bg-[#16324f]'}`} />
    ...
    {/* Right column */}
    <div className={`flex w-full shrink-0 flex-col items-center border-t border-dotted pt-4 lg:w-[210px] lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0 transition-colors duration-300 ${isLight ? 'border-[#E5E7EB]' : 'border-[#73869e]'}`}>
      <div className={`mt-1 h-[10px] w-[48px] rounded transition-colors duration-300 ${isLight ? 'bg-[#E5E7EB]' : 'bg-[#122844]'}`} />
      <div className={`mt-2 h-[20px] w-[70px] rounded transition-colors duration-300 ${isLight ? 'bg-[#D1D5DB]' : 'bg-[#1c3a5f]'}`} />
      <div className={`mt-3 h-[10px] w-[56px] rounded transition-colors duration-300 ${isLight ? 'bg-[#E5E7EB]' : 'bg-[#122844]'}`} />
      <div className={`mt-4 h-[36px] w-full rounded-[10px] lg:mt-auto transition-colors duration-300 ${isLight ? 'bg-[#2563EB]' : 'bg-[#1b4aa0]'}`} />
    </div>
  </div>
  );
};