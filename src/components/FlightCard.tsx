...
const SelectButton = ({ selected, onSelect, isLight }: { selected: boolean; onSelect?: () => void; isLight?: boolean }) => (
  <button
    ...
    className={`flex h-[24px] w-full cursor-pointer items-center justify-center gap-2 rounded-[10px] text-[12px] font-bold text-white transition-all duration-300 ${isLight ? 'bg-[#2563EB] hover:bg-[#1D4ED8] shadow-[0_4px_12px_rgba(37,99,235,0.25)] hover:shadow-[0_6px_16px_rgba(37,99,235,0.35)] active:bg-[#1D4ED8]' : selected ? 'bg-[#d4af37] shadow-[0_0_16px_rgba(212,175,55,0.5),0_0_30px_rgba(212,175,55,0.25)] hover:bg-[#f0c265] hover:shadow-[0_0_20px_rgba(212,175,55,0.7),0_0_40px_rgba(212,175,55,0.35)]' : 'bg-[#3b82f6] shadow-[0_4px_12px_rgba(59,130,246,0.25)] hover:bg-[#d4af37] hover:shadow-[0_0_16px_rgba(212,175,55,0.5),0_0_35px_rgba(212,175,55,0.25)] active:bg-[#d4af37] active:shadow-[0_0_22px_rgba(212,175,55,0.65),0_0_45px_rgba(212,175,55,0.35)]'}`}
  >
...
const CompactFlightLeft = ({
  ...
  return (
    <div className="min-w-0 flex-1 pr-3 pb-2 lg:pb-0">
      {/* Airline header row */}
      <div className="flex items-start gap-2.5">
        <div className="flex shrink-0 flex-col items-start">
          {f.via && (
            <span className={`mb-0.5 max-w-[72px] truncate text-[8.5px] font-semibold leading-none transition-colors duration-300 ${isLight ? 'text-[#6B7280]' : 'text-[#9eafc7]'}`}>
              via {viaList.join(', ')}
            </span>
          )}
          <AirlineLogo airline={f.airline} size="sm" />
          <div className={`mt-1 flex items-center gap-1 whitespace-nowrap text-[8.5px] transition-colors duration-300 ${isLight ? 'text-[#4B5563]' : 'text-white'}`}>
          ...
          <span className={`mt-1 whitespace-nowrap text-[11px] font-semibold transition-colors duration-300 ${isLight ? 'text-[#6B7280]' : 'text-[#9eafc7]'}`}>{f.code}</span>
        </div>
        <span className={`mt-1 min-w-0 truncate text-[13px] font-bold leading-tight tracking-wide transition-colors duration-300 ${isLight ? 'text-[#111827]' : 'text-white'}`}>{f.airline}</span>
      </div>
      ...
      <div className="mt-0 flex items-center justify-end">
        <div className="w-[68px] shrink-0 text-right">
          ...
          <div className={`mt-1 text-[10px] font-semibold leading-none transition-colors duration-300 ${isLight ? 'text-[#111827]' : 'text-white'}`}>{depCode}</div>
          <div className={`mt-0.5 text-[9px] leading-none transition-colors duration-300 ${isLight ? 'text-[#6B7280]' : 'text-[#a0a8b8]'}`}>{depTerm}</div>
        </div>

        <div className="relative mx-2 h-6 w-[120px] shrink-0 sm:w-[180px]">
        ...
        <div className="w-[68px] shrink-0 text-left">
          ...
          <div className={`mt-1 text-[10px] font-semibold leading-none transition-colors duration-300 ${isLight ? 'text-[#111827]' : 'text-white'}`}>{arrCode}</div>
          <div className={`mt-0.5 text-[9px] leading-none transition-colors duration-300 ${isLight ? 'text-[#6B7280]' : 'text-[#a0a8b8]'}`}>{arrTerm}</div>
        </div>
      </div>

      {/* Horizontal divider */}
      <div className="mt-1 border-t border-dashed border-[#73869e]" />

      {/* Metadata: second sub-row (layover / hop / CO2) */}
      <div className={`mt-1 text-[10px] transition-colors duration-300 ${isLight ? 'text-[#4B5563]' : 'text-white'}`}>
...
const CompactPriceCol = ({
  ...
  <div className={`flex w-full shrink-0 flex-col items-center border-t border-dotted pt-1 text-center lg:w-[150px] lg:border-t-0 lg:border-l lg:pl-3 lg:pt-0 transition-colors duration-300 ${isLight ? 'border-[#E5E7EB]' : 'border-[#73869e]'}`}>
    <div className={`text-[10px] font-semibold tracking-[0.1em] transition-colors duration-300 ${isLight ? 'text-[#6B7280]' : 'text-[#9baec7]'}`}>TRIP FIT</div>
    <PriceBreakdownPopover
      id={`${scope ?? 'flight'}:${f.code}`}
      breakdown={priceBreakdownOf(f.price + dayDelta)}
      className={`mt-0.5 text-[16px] font-bold leading-tight tracking-tight transition-colors duration-300 ${isLight ? 'text-[#111827]' : 'text-white'}`}
    >
...
export const FlightCard = ({
  ...
  >
    ...
    <div className="min-w-0 flex-1 pr-0 pb-1.5 lg:pr-6">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0 pb-1">
          <span className={`block text-[14px] font-bold leading-snug tracking-wide transition-colors duration-300 ${isLight ? 'text-[#111827]' : 'text-white'}`}>{f.airline}</span>
          <div className={`mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] ${isLight ? 'text-[#4B5563]' : 'text-white'}`}>
            <span className="flex items-center gap-1.5 whitespace-nowrap">
              <Clock className={`h-[13px] w-[13px] transition-colors duration-300 ${isLight ? 'text-[#6B7280]' : 'text-[#b6c3d5]'}`} />
              {f.duration}
            </span>
            ...
            <span className={`h-3 w-px transition-colors duration-300 ${isLight ? 'bg-[#E5E7EB]' : 'bg-[#315073]'}`} />
            <span className="flex items-center gap-1.5 whitespace-nowrap">
              <ShoppingBag className={`h-[13px] w-[13px] transition-colors duration-300 ${isLight ? 'text-[#6B7280]' : 'text-[#b6c3d5]'}`} />
              {f.baggage}
            </span>
          </div>
          {viaList.length > 0 ? (
            <span className="mt-1 block whitespace-nowrap text-[9px] font-semibold text-[#fdba74]">
              via {viaList.join(', ')}
            </span>
          ) : (
            <span className="mt-1 flex items-center gap-1 whitespace-nowrap text-[9px] font-semibold text-[#22c55e]">
      ...
      <div className="mt-2 flex items-center justify-end">
        <div className="w-[88px] shrink-0 text-left sm:w-[110px]">
          <div className={`text-[15px] font-bold leading-tight transition-colors duration-300 ${isLight ? 'text-[#111827]' : 'text-white'}`}>{f.departure.time}</div>
          <div className={`mt-0.5 text-[11px] transition-colors duration-300 ${isLight ? 'text-[#4B5563]' : 'text-[#9baec7]'}`}>{fromLabel ?? f.departure.airport}</div>
        </div>

        <div className="relative mx-2 h-7 w-[140px] shrink-0 sm:w-[220px]">
          ...
          {f.via ? (
            ...
            <>
              ...
              {viaList.map((city, i) => (
                <div
                  key={`v-${city}`}
                  className={`absolute top-1/2 max-w-[50%] -translate-x-1/2 translate-y-[20px] truncate px-0.5 text-[10px] transition-colors duration-300 ${isLight ? 'text-[#6B7280]' : 'text-[#9baec7]'}`}
                  style={{ left: `${((i + 0.5) / planeCount) * 100}%` }}
                >
                  {city}
                </div>
        ...
        <div className="w-[88px] shrink-0 text-right sm:w-[110px]">
          <div className={`text-[15px] font-bold leading-tight transition-colors duration-300 ${isLight ? 'text-[#111827]' : 'text-white'}`}>{f.arrival.time}</div>
          <div className={`mt-0.5 text-[11px] transition-colors duration-300 ${isLight ? 'text-[#4B5563]' : 'text-[#9baec7]'}`}>{toLabel ?? f.arrival.airport}</div>
        </div>
      </div>
...
<div className={`flex w-full shrink-0 flex-col items-center border-t border-dotted pt-1 text-center lg:w-[190px] lg:border-t-0 lg:border-l lg:pl-5 lg:pt-0 transition-colors duration-300 ${isLight ? 'border-[#E5E7EB]' : 'border-[#73869e]'}`}>
<div className={`text-[10px] font-semibold tracking-[0.1em] transition-colors duration-300 ${isLight ? 'text-[#6B7280]' : 'text-[#9baec7]'}`}>TRIP FIT</div>
      <PriceBreakdownPopover
        id={`${scope ?? 'flight'}:${f.code}`}
        breakdown={priceBreakdownOf(f.price + dayDelta)}
