  ...
  return (
    <div className={`rounded-lg border p-6 lg:p-7 transition-colors duration-300 ${isLight ? 'border-[#EEEEEE] bg-white' : 'border-[#214b7e] bg-[#0f172a]'}`}>
      ...
      <div className="mb-5 flex items-center justify-between">
        ...
        <span className={`text-[12px] transition-colors duration-300 ${isLight ? 'text-[#777]' : 'text-white/50'}`}>1 Adult &middot; Corporate Booker</span>
      </div>
      ...
      <div className={`rounded-[7px] p-5 transition-colors duration-300 ${isLight ? 'bg-[#F7F4F3]' : 'bg-[#0d1b2a]'}`}>
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className={`rounded px-2 py-0.5 text-[10px] font-semibold transition-colors duration-300 ${isLight ? 'bg-[#E1EFFB] text-[#004B7C]' : 'bg-[#12304f] text-[#7CC0FF]'}`}>
