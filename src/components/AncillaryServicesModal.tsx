...
/* Aircraft cabin interior — front cross-section visualization shown at the top
   of the seat tab (fuselage, overhead bins, windows, 3+3 seats, aisle). */
const CabinVisualization = ({ isLight }: { isLight: boolean }) => {
  const c = isLight
    ? {
        fuselage: '#ffffff', fuselageStroke: '#d7dce3',
        panel: '#eef1f5', bin: '#e2e7ee', binStroke: '#c6cedb',
        window: '#cfeaff', windowStroke: '#7fb8e6',
        seat: '#d8e2ef', seatStroke: '#b8c6d8', headrest: '#c9d6e6',
        armrest: '#aebfd6', aisle: '#f2e9d0', floor: '#cdd5df',
        led: '#ffe9a8', text: '#8a93a3', exit: '#E5484D',
      }
    : {
        fuselage: '#0f1a30', fuselageStroke: '#29466e',
        panel: '#16233f', bin: '#1c3a5f', binStroke: '#2d548a',
        window: '#2b6db3', windowStroke: '#59a7f0',
        seat: '#1c3a5f', seatStroke: '#2d548a', headrest: '#24548c',
        armrest: '#2d548a', aisle: '#182c4c', floor: '#29466e',
        led: '#4d6ea3', text: '#5f7296', exit: '#E5484D',
      };
  ...
  return (
    <div className={`overflow-hidden rounded-lg border ${isLight ? 'border-[#EEEEEE]' : 'border-[#29466e]'}`}>
      <svg
        viewBox="0 0 460 236"
        role="img"
        aria-label="Aircraft cabin interior"
        className="block w-full"
      >
        <ellipse cx="150" cy="70" rx="180" ry="60" fill="#ffffff" opacity={isLight ? 0.12 : 0.05} />
        <path
          d="M58 226 C36 196 24 130 56 82 C88 32 372 32 404 82 C436 130 424 196 402 226 Z"
          fill={c.fuselage}
          stroke={c.fuselageStroke}
          strokeWidth="1.5"
        />
        <path d="M84 78 C130 58 330 58 376 78" fill="none" stroke={c.fuselageStroke} strokeWidth="1" opacity="0.6" />
        <rect x="144" y="54" width="172" height="5" rx="2.5" fill={c.led} opacity="0.9" />
        <rect x="66" y="92" width="82" height="13" rx="6.5" fill={c.bin} stroke={c.binStroke} strokeWidth="1" />
        <rect x="172" y="92" width="116" height="13" rx="6.5" fill={c.panel} stroke={c.binStroke} strokeWidth="1" />
        <rect x="312" y="92" width="82" height="13" rx="6.5" fill={c.bin} stroke={c.binStroke} strokeWidth="1" />
        <circle cx="100" cy="124" r="8.5" fill={c.window} stroke={c.windowStroke} strokeWidth="1.5" />
        <circle cx="140" cy="106" r="6.5" fill={c.window} stroke={c.windowStroke} strokeWidth="1.5" />
        <circle cx="360" cy="124" r="8.5" fill={c.window} stroke={c.windowStroke} strokeWidth="1.5" />
        <circle cx="320" cy="106" r="6.5" fill={c.window} stroke={c.windowStroke} strokeWidth="1.5" />
        <rect x="46" y="120" width="16" height="7" rx="2" fill={c.exit} />
        <rect x="398" y="120" width="16" height="7" rx="2" fill={c.exit} />
        <rect x="172" y="148" width="56" height="56" rx="6" fill={c.aisle} opacity="0.85" />
        {seats.map((s) => (
          <g key={s.letter}>
            <rect x={s.x} y="148" width="32" height="56" rx="7" fill={c.seat} stroke={c.seatStroke} strokeWidth="1" />
            <rect x={s.x + 4} y="138" width="24" height="12" rx="5" fill={c.headrest} />
            <text x={s.x + 16} y="223" textAnchor="middle" fontSize="8" fontWeight="700" fill={c.text} fontFamily="inherit">
              {s.letter}
            </text>
          </g>
        ))}
        <rect x="90" y="150" width="3" height="50" fill={c.armrest} />
        <rect x="128" y="150" width="3" height="50" fill={c.armrest} />
        <rect x="166" y="150" width="4" height="50" fill={c.armrest} />
        <rect x="230" y="150" width="4" height="50" fill={c.armrest} />
        <rect x="318" y="150" width="3" height="50" fill={c.armrest} />
        <rect x="356" y="150" width="3" height="50" fill={c.armrest} />
        <line x1="54" y1="206" x2="406" y2="206" stroke={c.fuselageStroke} strokeWidth="2" opacity="0.8" />
      </svg>
    </div>
  );
};

/* Small decorative cabin window dot aligned to each seat row */
const seatWindowDot = (isLight: boolean) => (
  <span
    aria-hidden
    className={`h-2 w-2 shrink-0 rounded-full ${isLight ? 'bg-[#cfeaff] ring-1 ring-[#7fb8e6]' : 'bg-[#2b6db3] ring-1 ring-[#59a7f0]'}`}
  />
);
