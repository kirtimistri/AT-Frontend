...
export const FilterGroup = ({ icon, label, value, slider, options, active, onToggle, onOptionToggle }: FilterGroupProps) => {
  ...
  const cardCls = isLight
    ...
    : active
      ? 'bg-[rgba(212,175,55,0.12)] border-[rgba(212,175,55,0.45)]'
  ...
  return (
    ...
    >
      ...
      {options && active && (
        <div className="filter-expand flex flex-col gap-1 pt-1.5 pl-[34px]" onClick={(e) => e.stopPropagation()}>
          {options.map((opt, i) => (
            <button
              key={opt.label}
              type="button"
      ...
      {slider && active && (
        <div className="filter-expand pl-[34px] pt-1" onClick={(e) => e.stopPropagation()}>
          <RangeSlider {...slider} />
        </div>
      )}
    </div>
