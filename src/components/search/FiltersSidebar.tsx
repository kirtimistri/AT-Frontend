// FiltersSidebar.tsx – Exact match for Stops & Time filters with Golden Selection Glow.
...
const TimeGridButton = ({ range, iconType, selected, onClick }: TimeGridButtonProps) => {
  ...
  const btnCls = selected
    ? 'border-[#f0c265]/60 bg-[rgba(212,175,55,0.12)] text-white shadow-[0_0_14px_rgba(240,194,101,0.3)]'
    : 'border-white/10 bg-white/[0.03] text-white/70 hover:border-white/20 hover:bg-white/[0.06] hover:text-white';

  const iconCls = selected ? 'text-[#f0c265]' : 'text-white/50';
...
const FilterCheckbox = ({ label, sublabel, checked, onChange }: FilterCheckboxProps) => {
  ...
  const checkboxCls = checked
    ? 'border-[#f0c265] bg-[#d4af37] text-[#0E1833] shadow-[0_0_0_3px_rgba(240,194,101,0.18),0_2px_6px_rgba(240,194,101,0.4)] scale-105'
    : isLight
      ? 'border-[#D1D5DB] bg-white group-hover/row:border-[#f0c265]'
...
/* ── RangeSliderRed (Now with Golden Glow) ────────────────────────────── */

type RangeSliderRedProps = {
  fromPct?: number;
  toPct?: number;
  label: string;
};

const RangeSliderRed = ({ fromPct = 5, toPct = 55, label }: RangeSliderRedProps) => {
...
const PillInput = () => {
  const [value, setValue] = useState('4, 10, 152, 314');
  const { theme } = useThemeStore();
  const isLight = theme === 'light';
  ...
  const boxCls = isLight
    ? 'border-[#d4af37]/50 bg-[#d4af37]/10 focus-within:border-[#f0c265] focus-within:shadow-[0_0_0_3px_rgba(240,194,101,0.12)]'
...
export const FiltersSidebar = () => {
  ...
  const priceInputBorder = isLight ? 'border-[#E5E7EB] bg-white' : 'border-white/15 bg-white/[0.05]';
      ...
      <div className="hide-scrollbar min-h-0 flex-1 overflow-y-auto">
        <div key={resetKey}>
          ...
          <FilterSection icon={<Clock className="h-4 w-4" />} title="Departure Duration">
            <RangeSliderRed label="0hr - 1hr 45min" />
          </FilterSection>
          ...
          <FilterSection icon={<Tag className="h-4 w-4" />} title="Price">
            <RangeSliderRed label="₹8,237 - ₹30,130" />
